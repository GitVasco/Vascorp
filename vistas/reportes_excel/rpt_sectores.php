...
# Titulo de la hoja
$objPHPExcel->getActiveSheet()->setTitle("SECTORES -".$fecha);
...
// TITULO
$fila = 2;
$objPHPExcel->getActiveSheet()->SetCellValue("D$fila", 'SECTORES');
...
$objPHPExcel->getActiveSheet()->SetCellValue("B$fila", 'COD. SECTOR');
...
$objPHPExcel->getActiveSheet()->SetCellValue("C$fila", 'SECTOR');
...
while($respDetalle = mysql_fetch_array($sqlDetalle)){
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue("B$fila", utf8_encode($respDetalle["cod_sector"])); 
    
    $objPHPExcel->getActiveSheet()->SetCellValue("C$fila", utf8_encode($respDetalle["nom_sector"]));
...
# Nombre del archivo
header('Content-Disposition: attachment; filename=" SECTORES - '.$fecha.'.xls"');
