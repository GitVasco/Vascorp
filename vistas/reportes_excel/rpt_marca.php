...
# Titulo de la hoja
$objPHPExcel->getActiveSheet()->setTitle("MARCAS -".$fecha);
...
// TITULO
$fila = 2;
$objPHPExcel->getActiveSheet()->SetCellValue("G$fila", 'MARCAS');
...
$objPHPExcel->getActiveSheet()->SetCellValue("C$fila", 'VENTA');
...
$sqlDetalle = mysql_query("SELECT * FROM marcasjf") or die(mysql_error());
...
while($respDetalle = mysql_fetch_array($sqlDetalle)){
    ...
    if($respDetalle["venta"] == 1){
    $objPHPExcel->getActiveSheet()->SetCellValue("C$fila", utf8_encode("ACTIVO"));
    }else{
        $objPHPExcel->getActiveSheet()->SetCellValue("C$fila", utf8_encode("INACTIVO"));
    }
...
# Nombre del archivo
header('Content-Disposition: attachment; filename=" MARCAS - '.$fecha.'.xls"');
