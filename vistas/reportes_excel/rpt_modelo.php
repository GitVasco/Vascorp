...
# Titulo de la hoja
$objPHPExcel->getActiveSheet()->setTitle("MODELOS -".$fecha);
...
// TITULO
$fila = 2;
$objPHPExcel->getActiveSheet()->SetCellValue("G$fila", 'MODELOS');
...
$objPHPExcel->getActiveSheet()->SetCellValue("F$fila", 'TIPO');
...
$sqlDetalle = mysql_query("SELECT t.id_modelo,t.modelo,t.nombre,t.estado,t.tipo,t.linea,t.operaciones,t.articulos,m.marca FROM modelojf t LEFT JOIN marcasjf m ON t.id_marca=m.id ORDER BY id_modelo ASC") or die(mysql_error());
...
while($respDetalle = mysql_fetch_array($sqlDetalle)){
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue("F$fila", utf8_encode($respDetalle["tipo"]));

    $objPHPExcel->getActiveSheet()->SetCellValue("G$fila", utf8_encode($respDetalle["linea"]));

    if($respDetalle["operaciones"] == 1){
    $objPHPExcel->getActiveSheet()->SetCellValue("H$fila", utf8_encode("Si tiene"));
    }else{
        $objPHPExcel->getActiveSheet()->SetCellValue("H$fila", utf8_encode("Pendiente"));
    }
...
$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(18.72);
...
# Nombre del archivo
header('Content-Disposition: attachment; filename=" MODELOS - '.$fecha.'.xls"');
