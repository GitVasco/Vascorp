...
//rellenar con contenido
    ...
    $sql=mysql_query("SELECT   DISTINCT vd.Item,pro.CodFab, pro.CodPro,TbCol.Des_Larga AS DesCol,
            TbPai.Des_Larga AS DesPai,TbUnd.Des_Larga AS DesUnd, vd.CanVta,pro.DesPro,pro.ColPro,pro.CosPro,
             vd.CodPro, vd.PreVta, vd.CanVta, 
          CASE WHEN TbAmp.Des_Larga IS NULL THEN ''
            ELSE TbAmp.Des_Larga
            END AS Destino
    FROM    Venta_Det vd
    INNER JOIN Producto  pro ON
         pro.CodPro= vd.CodPro
    LEFT JOIN Tabla_M_Detalle AS TbCol ON
         TbCol.Cod_Tabla IN ('TCOL')
         AND TbCol.Cod_Argumento=pro.ColPro
    LEFT JOIN Tabla_M_Detalle AS TbPai ON
         TbPai.Cod_Tabla IN ('PAIS')
         AND TbPai.Cod_Argumento=pro.PaiPro
    LEFT JOIN Tabla_M_Detalle AS TbUnd ON 
         TbUnd.Cod_Tabla IN ('TUND')
         AND TbUnd.Cod_Argumento=pro.UndPro
    LEFT JOIN Tabla_M_Detalle AS TbAmp ON
         TbAmp.Cod_Tabla IN ('TAMP')
         AND TbAmp.Cod_Argumento=vd.CenCosto
    WHERE   vd.Nro=$id order by Item Asc ");
