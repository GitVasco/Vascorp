...
if (localStorage.getItem("capturarRango28") != null) {
    $("#daterange-btnNotasSalidas span").html(
        localStorage.getItem("capturarRango28")
    );
    cargarTablaNotasSalidas(
...
function cargarTablaNotasSalidas(fechaInicial, fechaFinal) {
    $(".tablaNotasSalidas").DataTable({
        ajax:
            "ajax/materiaprima/tabla-notas-salidas.ajax.php?perfil=" +
            $("#perfilOculto").val() +
            "&fechaInicial=" +
            fechaInicial +
            "&fechaFinal=" +
...
$(".tablaMateriaNotaSalida").DataTable({
    ajax:
        "ajax/materiaprima/tabla-materias-notas-salidas.ajax.php?perfil=" +
        $("#perfilOculto").val(),
    deferRender: true,
    retrieve: true,
    processing: true,
...
$(".tablaMateriaNotaSalida tbody").on(
    "click",
    "button.agregarMateriaNota",
    function () {
      ...
      =============================================*/
                ...
                $.ajax({
                    ...
                    success: function (respuesta2) {
                        $("#nuevoDestinoNota" + codpro)
                            .find("option")
                            .remove();

                        $("#nuevoDestinoNota" + codpro).append(
                            "<option value=''>SELECCIONAR DESTINO</option>"
                        );
                        for (let i = 0; i < respuesta2.length; i++) {
                            $("#nuevoDestinoNota" + codpro).append(
                                "<option value='" +
                                    respuesta2[i]["cod_area"] +
                                    "'>" +
                                    respuesta2[i]["cod_area"] +
                                    " - " +
                                    respuesta2[i]["nombre_area"] +
                                    "</option>"
                            );
                        }
                    },
                ...
                $(".nuevaMateriaNota").append(
                    '<div class="row" style="padding:1px 15px">' +
                        ...
                        "</div>" +
                        '<div class="col-xs-1" >' +
                        '<input type="text" class="form-control input-sm nuevoCodigoColor"  name="nuevoCodigoColor" value="' +
                        codcolor +
                        '"  readonly>' +
                        "</div>" +
                        '<div class="col-xs-1" >' +
                        ...
                        "</div>" +
                        '<div class="col-xs-1" >' +
                        '<input type="text" class="form-control input-sm nuevoCosto"  name="nuevoCosto" value="' +
                        precio +
                        '"  readonly required>' +
                        "</div>" +
                        '<div class="col-xs-2" >' +
                        '<select class="form-control input-sm nuevoDestinoNota" name="nuevoDestinoNota" id="nuevoDestinoNota' +
                        codpro +
                        '" required>' +
                        '<option value="">Seleccionar destino</option>' +
                        "</select>" +
                        ...
                        Number(stock - 1) +
                        '" required>' +
                        '<span class="input-group-addon"  style="padding: 3px 6px"><button type="button" class="btn btn-danger btn-xs quitarMateriaNota" idMateriaNota="' +
                        idMateriaNota +
...
$(".tablaMateriaNotaSalida").on("draw.dt", function () {
    ...
    if (localStorage.getItem("quitarMateriaNota") != null) {
        var listaIdMateriaNota = JSON.parse(
...
$(".formularioNotaSalida").on("click", "button.quitarMateriaNota", function () {
  ...
  =============================================*/
    ...
    localStorage.setItem(
        "quitarMateriaNota",
        JSON.stringify(idQuitarMateriaNota)
    );

    $(
        "button.recuperarBoton[idMateriaNota='" + idMateriaNota + "']"
    ).removeClass("btn-default");
...
function quitarAgregarMateriaNota() {
    ...
    var botonesTabla = $(
        ".tablaMateriaNotaSalida tbody button.agregarMateriaNota"
    );
...
$(".tablaNotasSalidas").on("click", ".btnAnularNotaSalida", function () {
    ...
    swal({
        ...
        if (result.value) {
            window.location =
                "index.php?ruta=notas-salidas&idNotaSalida=" + idNotaSalida;
        }
    });
});
 ...
 */
$(".tablaNotasSalidas").on(
    "click",
    ".btnDetalleReporteNotaSalida",
    function () {
        var idNotaSalida = $(this).attr("idNotaSalida");
        //console.log("codigo", codigo);

        window.location =
            "vistas/reportes_excel/rpt_notasalida.php?idNotaSalida=" +
            idNotaSalida;
    }
);
        ...
        success: function (respuesta) {
            ...
            if (estadoNotaSalida == "1") {
                Command: toastr["success"](
                    "Nota de salida aprobada correctamente!"
                );
            }
        },
    });
...
$(".formularioUnirNotaSalida").on(
    "change",
    "select#selectNotaSalida",
    function () {
...
$(".formularioUnirNotaSalida").on(
    "change",
    "select#selectDependienteNotaSalida",
    function () {
...
$(".formularioUnirNotaSalida").on(
    ...
    =============================================*/
            ...
            Command: toastr["error"](
                "Solo hay " + $(this).attr("saldo") + " de saldo disponible"
            );
        }
    }
);
