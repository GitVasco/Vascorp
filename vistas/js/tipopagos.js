$(".tablaTipoPagos").DataTable({
    ajax:
        "ajax/maestros/tabla-tipopagos.ajax.php?perfil=" +
        $("#perfilOculto").val(),
    deferRender: true,
    retrieve: true,
    processing: true,
...
$(".tablaTipoPagos").on("click", ".btnEliminarTipoPago", function () {
    ...
    swal({
        ...
        if (result.value) {
            window.location =
                "index.php?ruta=tipopagos&idTipoPago=" + idTipoPago;
        }
    });
});
//Reporte de Colores
...
$(".box").on("click", ".btnRptTipoPago", function () {
    window.location = "vistas/reportes_excel/rpt_tipo_pago.php";
});
