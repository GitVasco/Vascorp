...
class TablaGuiasRemision
{
    ...
    public function mostrarTablaGuiasRemision()
    {
        ...
        "data": [';
            ...
            *estado
            */
                $facturacion = $factura[$i]["facturacion"];
                $hoyFecha = $factura[$i]["fecha"];
                $documento = $factura[$i]["documento"];
                $tipo = $factura[$i]["tipo"];

                switch ($facturacion) {
                    case "0":
                        $label = $hoy == $hoyFecha ? 'success' : 'warning';
                        $estado = "<span style='font-size:85%' class='label label-$label'>GENERADO</span>";
                        break;
                    case "1":
                    ...
                    case "2":
                        ...
                        break;
                    case "4":
                        $estado = "<span class='btn btn-danger btn-xs btn btnEliminarDocumento' documento='$documento' tipo='$tipo' pagina='facturas'>BAJA</span>";
                        break;
                }
